...
<body>
	<div class="container-fluid">
		...
		<div class="row">
			...
			<div class="col">
				<form:form action="/chains/new" method="post" modelAttribute="newChain">
					<p class="fs-4">Add new chain</p>
					<div class="row my-4">
						<form:label path="name" class="col-2">Name:</form:label>
						<form:input path="name" class="col-6"/>
						<form:errors path="name" class="text-danger offset-2"/>
					</div>
					<input type="submit" class="btn btn-primary offset-2 col-2" value="Add chain">
				</form:form>
			</div>
		</div>
		<p class="fs-1">All chains</p>
		<table class="table table-striped table-hover">
			<thead>
				<tr>
					<th>ID</th>
			...
			<tbody>
				<c:forEach var="chain" items="${chains}">
					<tr class="align-middle">
						<td><c:out value="${chain.id}"/></td>
						<td><c:out value="${chain.name}"/></td>
						<td>
							<a href="/chains/${chain.id}" class="btn btn-primary">View</a>
							<a href="/chains/${chain.id}/delete" class="btn btn-danger">Delete</a>
						</td>
					</tr>
				</c:forEach>
			</tbody>
